...
def my_sem_analysis(data, model_spec, output_file=None, output_dir=None, file_prefix=None, display_results=None):
    """
    ...
    display_results : bool, optional
        Whether to display results in Jupyter notebook. If None, will auto-detect environment.
    ...
    """
    # Auto-detect environment if display_results is not specified
    if display_results is None:
        display_results = JUPYTER_AVAILABLE
    ...
    # Print info function (works in both environments)
    def print_info(msg, heading_level=3):
        if JUPYTER_AVAILABLE and display_results:
            prefix = "#" * heading_level
            display(Markdown(f"{prefix} {msg}"))
        else:
            # Print plain text with some formatting
            separator = "=" * len(msg) if heading_level <= 2 else "-" * len(msg)
            print("\n" + separator)
            print(msg)
            print(separator + "\n")
    ...
    try:
        ...
        results['pca'] = {
            ...
            'factor_loadings_path': factor_loadings_path
        }
    except Exception as e:
    ...
    # Create detailed model graph with parameter estimates
    try:
        G = nx.DiGraph()
    ...
    # Display results in Jupyter notebook if available and requested
    if display_results and JUPYTER_AVAILABLE:
        try:
    ...
    # Print summary in regular Python environment if not in Jupyter but display_results is True
    elif display_results:
        print_info("Structural Equation Modeling Analysis Results", heading_level=1)
        print_info("Model Fit", heading_level=2)
        if 'fit_indices' in results and results['fit_indices']:
            fit = results['fit_indices']
        ...
        print_info("Parameter Estimates", heading_level=2)
        if 'parameters' in results and results['parameters'] is not None:
            print(results['parameters'].to_string(index=False))
        
        print_info("Effects", heading_level=2)
        if 'effects' in results and results['effects'] is not None and not effects_df.empty:
            print(effects_pivoted.to_string(index=False))
        
        print_info("Reliability", heading_level=2)
        if 'cronbach_alpha' in results and results['cronbach_alpha']:
            for lv, alpha in results['cronbach_alpha'].items():
                print(f"{lv}: Cronbach's alpha = {alpha:.3f}")
        
        print("\nOutput files saved to:")
        print(f"- Path Diagram: {path_diagram_path}")
        print(f"- Effects Plot: {results.get('effects', {}).get('plot_path', 'Not available')}")
        print(f"- Effects Decomposition: {results.get('effects', {}).get('decomposition_path', 'Not available')}")
        print(f"- Scree Plot: {scree_plot_path}")
        print(f"- Factor Loadings: {factor_loadings_path}")
    
    return results

# 일반 Python 스크립트에서 사용 예시
if __name__ == "__main__":
    print("SEM Analysis module loaded. Use my_sem_analysis() function to perform analysis.")
    print("Example usage:")
    print("""
    import pandas as pd
